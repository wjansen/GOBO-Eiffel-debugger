...
feature -- Basic operation 
	...
	save_system (a_target_system: DG_SYSTEM)
		...
		local
			...
			l_value_name, l_filename, l_name: STRING
		do
			l_name := compilee.name
			l_filename := l_name + "0"
			create l_file.make_open_write (l_filename + ".c")
			c_generator.c_filenames.force_last (".c", l_filename)
			l_file.put_string ("#include %"")
			l_file.put_string (l_name)
			l_file.put_string (".h%"%N%N")
			print_extension
			put_declarations
			l_value_name := "x"
			...
			create l_target.make (l_file, "", l_value_name,
...
--			Precursor
			...
			print_forward_variable (True, l_type.c_name, c_names.c_rts_name, Void)
...
#define DllExport extern 
			...
			-- Define `zt' etc.
			c_file.put_string ("void* gedb_t = GE_zt;%N")
			c_file.put_string ("void* gedb_o = GE_zo;%N")
			c_file.put_string ("void* gedb_ov = GE_zov;%N")
			c_file.put_string ("void* gedb_ms = GE_zms;%N")
...
feature -- Print C structs 
	...
	put_declarations
		...
		do
			...
			l_type := c_generator.debugger.type_by_name("TYPE", True)
			l_string := l_type.c_name
			c_file.put_string("return ((GedbSystem*)")
			...
			l_string := l_type.c_name
			c_file.put_string(l_string)
			c_file.put_string("* t = ((GedbSystem*)")
			c_file.put_string(c_names.c_rts_name)
